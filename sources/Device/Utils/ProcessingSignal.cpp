...
float CalculateVoltageMax(Channel ch)
{
    ...
    return POINT_2_VOLTAGE((uint8)ROUND(max), RANGE_DS(ch), RSHIFT_DS(ch));
}
...
float CalculateVoltageMin(Channel ch)
{
    ...
    return POINT_2_VOLTAGE((uint8)ROUND(min), RANGE_DS(ch),RSHIFT_DS(ch));
}
...
float CalculateVoltageVybrosPlus(Channel ch)
{
    ...
    return fabsf(POINT_2_VOLTAGE((uint8)ROUND(maxSteady), RANGE_DS(ch), rShift) - POINT_2_VOLTAGE((uint8)ROUND(max), RANGE_DS(ch), rShift));
}
...
float CalculateVoltageVybrosMinus(Channel ch)
{
    ...
    uint16 rShift = RSHIFT_DS(ch);
    return fabsf(POINT_2_VOLTAGE((uint8)ROUND(minSteady), RANGE_DS(ch), rShift) - POINT_2_VOLTAGE((uint8)ROUND(min), RANGE_DS(ch), rShift));
}
...
float CalculatePeriod(Channel ch)
{
    ...
    if ((MEAS_MARKED == Meas_Period || MEAS_MARKED == Meas_Freq) && periodIsCaclulating[ch])
    {
...
float CalculateDurationPlus(Channel ch)
{
    ...
    if (MEAS_MARKED == Meas_DurationPlus)
    {
        markerTime[ch][0] = (int)((int16)firstIntersection - firstByte);
        markerTime[ch][1] = (int)((int16)secondIntersection - firstByte);
    }
...
float CalculateDurationMinus(Channel ch)
{
    ...
    if (MEAS_MARKED == Meas_DurationMinus)
    {
        markerTime[ch][0] = (int)((int16)firstIntersection - firstByte);
        markerTime[ch][1] = (int)((int16)secondIntersection - firstByte);
    }
...
void Processing::CountedRange(Channel ch)
{
    ...
    for (int i = 0; i < numBytes; ++i)
    {
        ...
        if (d)
        {
            float abs = POINT_2_VOLTAGE((uint8)d, rangeIn, (uint16)rShiftIn);
