...
bool FPGA::ProcessingData()
{
    ...
    for (int i = 0; i < num; i++)
    {
        ...
        {
            timeCompletePredTrig = COUNT_MS;         // записываем время, когда оно произошло.
        }
        ...
        if (i > 0)
        {
            ...
            while (_GET_BIT(flag, FL_TRIG_READY) == 0 && _GET_BIT(flag, FL_DATA_READY) == 0 && (COUNT_MS - time) < 10)
        ...
        {
            if (COUNT_MS - timeCompletePredTrig > TSHIFT_2_ABS(2, SET_TBASE) * 1000) // Если прошло больше времени, чем помещается в десяти клетках
            {
                if (IN_P2P_MODE)
                {
