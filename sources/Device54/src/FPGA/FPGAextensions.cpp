...
int16 FPGA::CalculateAdditionRShift(Channel ch, Range range, bool wait)
{
    ...
    for(int i = 0; i < numMeasures; i++)
    {
        ...
        if(COUNT_MS - startTime > timeWait)          // Если прошло слишком много времени -
        {
...
void FPGA::CalibrateAddRShift(Channel ch, bool wait)
{
    ...
    for (int i = 0; i < 3; i++)                         // Сохранямем ручные смещения для последующего восстановления
    {
        add[i] = RSHIFT_ADD_STABLE(ch, i);
    }

    for (int i = 0; i < 3; i++)                         // Обновляем ручные смещения
    {
...
void FPGA::CalibrateChannel(Channel ch)
{
    ...
    {
        if (Panel::WaitPressingButton() == Key::Start)             // Ожидаем подтверждения или отмены процедуры калибровки канала.
        {
