...
void FLASH_Prepare();                               ///< Очищает сектора, предназначенные для записи прошивки.
void WriteData(uint address, uint8 *data, int size);    ///< Записать буфер, на который указывает указатель data, размером size, по адресу address.
